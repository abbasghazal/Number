...
async def search_number(event):
    async with event.client.conversation(event.chat_id) as conv:
        await conv.send_message(msgs['SEARCH_PROMPT'])
        try:
            query = await conv.get_response(timeout=60)
            results = await search_accounts(query.text)
            
            if not results:
                # إذا لم توجد نتائج، إضافة دولة جديدة
                await conv.send_message(msgs['COUNTRY_NOT_FOUND'])
                country_data = await conv.get_response(timeout=60)
                try:
                    name, code = country_data.text.split()
                    await add_new_country(name, code)
                    await conv.send_message(msgs['COUNTRY_ADDED'])
                except:
                    await conv.send_message("❌ تنسيق غير صحيح!")
                return
                
            # عرض نتائج البحث
            response = ""
            for account in results:
                response += f"• الدولة: {account[3]}\n• الرقم: +{account[1]}\n\n"
                
            await event.edit(msgs['SEARCH_RESULTS'].format(response))
            
        except asyncio.TimeoutError:
            await event.answer("⏱ انتهى الوقت المحدد!")

async def buy_number(event):
    user_id = event.chat_id
    countries = await get_countries(only_with_accounts=True)  # عرض الدول ذات الأرقام فقط
...
async def select_account(event, data):
    ...
    buttons = [
        [Button.inline("• تأكيد الشراء • ✅", data=f"buy_{calling_code}_{phone}")],
